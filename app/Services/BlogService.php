...
use App\Actions\Blog\BulkAction;
use App\Actions\Blog\CreateAction;
use App\Actions\Blog\DeleteAction;
use App\Actions\Blog\RestoreAction;
use App\Actions\Blog\UpdateAction;
...
class BlogService
{
    public function __construct(
        protected BlogRepositoryInterface $interface,
    ...
    * ================== ================== ================== */
    ...
    public function createData(array $data): Blog
    {
        return $this->createAction->execute(data: $data);
    }

    public function updateData(int $id, array $data): Blog
    {
    ...
    public function updateStatusData(int $id, BlogStatus $status, ?int $actionerId = null): Blog
    {
        if ($actionerId == null) {
            $actionerId = admin()->id;
        }
    ...
    public function bulkUpdateStatus(array $ids, BlogStatus $status, ?int $actionerId = null): int
    {
        if ($actionerId == null) {
            $actionerId = admin()->id;
        }
    ...
    * ================== ================== ================== */
    ...
    {
        return $this->interface->getPublished($sortField, $order);
    }

    public function getInactiveData($sortField = 'created_at', $order = 'desc'): Collection
    {
        return $this->interface->getUnpublished($sortField, $order);
    }
}
