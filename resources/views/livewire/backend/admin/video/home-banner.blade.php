<section>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-4 lg:p-6 mb-6">
        <div class="">
            ...
            <form wire:submit="updateFooterBanner" class="space-y-6">
                <!-- Add other form fields here -->
                <div class="mt-6 space-y-4 grid grid-cols-2 gap-5">
                    <div class="w-full">
                        <x-ui.file-input wire:model="form.footer_thumbnail" label="{{ __('Footer Thumbnail') }}" accept="image/*"
                            :existingFiles="$existingFooterThumbnail" removeModel="form.remove_file"
                            hint="Upload a profile picture (Max: 2MB)" />
                        <x-ui.input-error :messages="$errors->get('form.footer_thumbnail')" />
                    </div>
                    <div class="w-full">
                        <x-ui.file-input wire:model="form.footer_file" label="{{ __('Footer Video File') }}" accept="video/*"
                            :existingFiles="$existingFooterFile" removeModel="form.remove_file" hint="Upload a video file (Max: 10MB)" />
                        <x-ui.input-error :messages="$errors->get('form.footer_file')" />
                    </div>
                </div>
