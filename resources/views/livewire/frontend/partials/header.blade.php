<header x-data="{ mobileMenuOpen: false }" x-cloak class="fixed top-0 left-0 right-0 z-50 bg-transparent pt-4">
    <div class="container flex items-center justify-between py-3 px-6">
        ...
        <nav class="hidden md:flex items-center bg-second-900/90 rounded-full px-2 py-2 gap-1">
            ...
            <div class="relative" x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false">
                ...
                <div x-show="open" x-transition x-cloak
                    ...
                    <a href="{{ route('gallery') }}" @click="open = false" class="block text-center px-4 py-1.5 rounded-full text-base {{ request()->routeIs('gallery') ? 'bg-second-500 text-white' : 'bg-white' }}">
                        {{ __('Gallery') }}
                    </a>
        ...
        <!-- Mobile Menu Button -->
        <button @click="mobileMenuOpen = !mobileMenuOpen" class="md:hidden p-2 text-white" aria-label="{{ __('Toggle menu') }}">
    ...
    <div x-show="mobileMenuOpen" x-transition
        ...
        <nav class="flex flex-col gap-4">
            ...
            <div x-data="{ open: false }" class="flex flex-col">
                ...
                <div x-show="open" x-transition x-cloak class="mt-2 ml-4 flex flex-col gap-3">
                    <a href="#" class="text-white hover:text-second-500 transition">{{ __('Gallery') }}</a>
                    <a href="#" class="text-white hover:text-second-500 transition">{{ __('Methods') }}</a>
                    <a href="#" class="text-white hover:text-second-500 transition">{{ __('FAQ') }}</a>
                </div>
            </div>
